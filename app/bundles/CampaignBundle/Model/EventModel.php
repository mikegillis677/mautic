...
class EventModel extends CommonFormModel
{
    ...
    public function triggerNegativeEvents(
        ...
        list($campaignEvents, $nonActionEvents) = $this->getCampaignEvents($campaignId);
        ...
        $start               = $executedEventCount               = $evaluatedEventCount               = $negativeExecutedCount               = $negativeEvaluatedCount               = 0;
        ...
        if ($leadCount) {
            ...
            while ($start <= $leadCount) {
                ...
                if ($this->queueService->isQueueEnabled()) {
                    ...
                    $this->queueService->publishToQueue(QueueName::NEGATIVE_EVENTS_TRIGGER, $msg);

                    $totalEventCount += $limit;
                    $negativeEvaluatedCount += $limit;
                } else {
                    $counts = $this->triggerNegativeEvent(
                        $campaignId,
                        $start,
                        $limit,
                        $max,
                    ...
                    if ($returnCounts) {
                        $nonActionEventCount += $counts['events'];
                        $negativeEvaluatedCount += $counts['evaluated'];
                        $negativeExecutedCount += $counts['executed'];
     ...
     * @return array
     */
    protected function getCampaignEvents($campaignId)
    {
        $campaignEvents = $this->getRepository()->getCampaignEvents($campaignId);
        ...
        return [$campaignEvents, $nonActionEvents, $actionEvents];
    }

    public function triggerNegativeEvent(
        $campaignId,
        $start,
        $limit,
        $max,
        ...
        list($campaignEvents, $nonActionEvents, $actionEvents) = $this->getCampaignEvents($campaignId);
        ...
        foreach ($nonActionEvents as $parentId => $events) {
            ...
            foreach ($leads as $lead) {
                ...
                $this->logger->debug('CAMPAIGN: contact ID #'.$lead->getId().'; #'.$leadDebugCounter);
